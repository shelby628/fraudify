...
export default function UserTransactionsTable({ transactions }) {
  return (
    <div style={{
      overflowX: "auto",
      borderRadius: "12px",
      boxShadow: "0 4px 14px rgba(71,60,51,0.15)",
      background: "#473C33",
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", color: "#f3f4f6" }}>
        <thead style={{ background: "#3A312A", borderBottom: "2px solid #FDA769" }}>
          <tr>
            <th style={{ padding: "16px", textAlign: "left", fontSize: "14px", fontWeight: "600", color: "#FEC868" }}>Transaction ID</th>
            <th style={{ padding: "16px", textAlign: "left", fontSize: "14px", fontWeight: "600", color: "#FEC868" }}>Date and Time</th>
            <th style={{ padding: "16px", textAlign: "left", fontSize: "14px", fontWeight: "600", color: "#FEC868" }}>Amount</th>
            <th style={{ padding: "16px", textAlign: "left", fontSize: "14px", fontWeight: "600", color: "#FEC868" }}>Status</th>
          </tr>
        </thead>
        <tbody>
          {transactions.map((tx, i) => {
            const ds = getStatusStyle(tx.decision);
            return (
              <tr
                key={tx.id}
                style={{
          ...
          {transactions.length === 0 && (
            <tr>
              <td colSpan="4" style={{ padding: "32px", textAlign: "center", color: "#FEC868" }}>
